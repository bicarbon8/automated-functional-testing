...
export type SauceLabsBrowserSessionOptions = Merge<BrowserSessionOptions, {
    username?: string;
    accessKey?: string;
    tunnel?: boolean;
...
export class SauceLabsBrowserSession extends BrowserSession<SauceLabsBrowserSessionOptions> {
    ...
    get tunnel(): boolean {
        if (!this._tunnel) {
            this._tunnel = this.option('tunnel');
        }
        return this._tunnel;
    }
