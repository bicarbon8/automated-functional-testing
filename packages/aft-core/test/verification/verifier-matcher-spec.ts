import { between, containing, equaling, equivalent, exactly, greaterThan, matchingProps, havingValue, lessThan, not, havingProps } from "../../src";
...
describe('VerifyMatcher', () => {
    ...
    describe('MatchingProperties', () => {
        ...
        testData.forEach((data) => {
            it(`can get expected result from comparison: ${JSON.stringify(data)}`, () => {
                expect(matchingProps(data.expected).setActual(data.actual).compare()).toBe(data.result);
            });
        });
    });
    ...
    describe('HavingProperties', () => {
        const testData = [
            {expected: null, actual: {foo: 'bar'}, result: true},
            {expected: [], actual: {}, result: true},
            {expected: ['foo'], actual: {foo: 'bar'}, result: true},
            {expected: [['foo','string']], actual: {foo: 'bar', baz: 'foo'}, result: true},
            {expected: [['foo','object'],['baz','string']], actual: {foo: {foo: 'bar'}, baz: 'foo'}, result: true},
            {expected: [], actual: 'foo', result: false},
            {expected: null, actual: null, result: false},
            {expected: ['foo'], actual: {bar: 'foo'}, result: false},
            {expected: [['foo', 'string']], actual: {foo: {bar: 'bar'}}, result: false},
            {expected: [['bar', 'number']], actual: {bar: 'foo'}, result: false},
        ];
        testData.forEach((data) => {
            it(`can get expected result from comparison: ${JSON.stringify(data)}`, () => {
