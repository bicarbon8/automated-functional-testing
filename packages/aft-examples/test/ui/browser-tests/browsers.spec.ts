    ...
    it('can recover from StaleElementExceptions automatically', async function() {
        ...
        await verifyWithBrowser(async (tw: BrowserVerifier) => {
            ...
            await verify(async (v: Verifier) => {
                await v.logger.step('navigate to LoginPage');
                await loginPage.navigateTo();
                return await loginPage.session.driver.getCurrentUrl();
            }).withLogManager(tw.logger).and.withTestIds('C4567')
            .returns(containing('the-internet.herokuapp.com/login'));
            
            await verify(async (v: Verifier) => {
                await v.logger.step('click login button...');
                await loginPage.content().then(c => c.getLoginButton()).then(button => button.click());
                await v.logger.info('no exception thrown on click');
            }).withLogManager(tw.logger).and.withTestIds('C5678');

            await verify(async (v: Verifier) => {
                await v.logger.step('refresh page...');
                await tw.session.refresh();
                await v.logger.info('page refreshed');
            }).withLogManager(tw.logger).and.withTestIds('C6789');

            await verify(async (v: Verifier) => {
                await tw.logger.step('click login button after refresh...');
                await loginPage.content().then(c => c.getLoginButton()).then(button => button.click());
                await tw.logger.info('no exception thrown on click');
            }).withLogManager(tw.logger).and.withTestIds('C7890');
        }).withLogManager(aft.logMgr);
    });
